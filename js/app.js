(() => {
        ...
        if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
                ...
                if (el.closest("[data-spoller]")) {
                    ...
                    if ("A" === !el.tagName.toUpperCase()) e.preventDefault();
                }
            }
    ...
    da.init();
    let catTitle = document.querySelector(".category_title");
    if (catTitle) {
        let catTitleWidth = catTitle.offsetWidth;
        let boxArrows = document.querySelector(".category_slider_arrows");
        let boxArrowsWidth = boxArrows.offsetWidth;
        let newWidth = catTitleWidth - boxArrowsWidth;
        let catSliders = document.querySelectorAll(".category__slider");
        function sendWidth() {
            for (let index = 0; index < catSliders.length; index++) {
                const catSlider = catSliders[index];
                catSlider.style.width = newWidth + "px";
            }
        }
        sendWidth();
        let boxsArrows = document.querySelectorAll(".category_slider_arrows");
        if (boxsArrows) document.addEventListener("DOMContentLoaded", (function() {
            setTimeout((function() {
                for (let index = 0; index < boxsArrows.length; index++) {
                    const boxArrows = boxsArrows[index];
                    let firstBlock = boxArrows.children[0].className.includes("swiper-button-lock");
                    let secondBlock = boxArrows.children[1].className.includes("swiper-button-lock");
                    if (secondBlock && firstBlock) {
                        boxArrows.parentNode.style.width = catTitleWidth + "px";
                        boxArrows.remove();
                    }
                }
            }), 1e3);
